...
agent_ids=$(docker node ls --quiet --filter node.label=kpl.agent=true)
[ -n "$agent_ids" ] || fail 'At least two kpl.agent=true Linux, Ready, Active nodes are required.'
...
[ "$eligible" -ge 2 ] || fail 'At least two kpl.agent=true Linux, Ready, Active nodes are required.'

printf 'PASS: Swarm manager, pinned control node, attachable overlay, %s eligible Agents, image digest and capacity %s.\n' "$eligible" "$capacity"
