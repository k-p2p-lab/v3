...
[English](linux-deployment.md) | 한국어
...
Controller를 관리망 주소에 직접 게시해야 한다면 `.env`의 `KPL_BIND_ADDRESS`와 `KPL_HTTP_PORT`를 지정하십시오. `KPL_API_TOKEN`은 변경 API를 보호하며 GET 조회를 인증하는 설정은 아닙니다. Grafana 관리자 비밀번호와 읽기 전용 익명 접속 설정은 [모니터링 가이드](monitoring.kr.md)를 참고하십시오.
...
Swarm 다중 서버에서는 [전용 배포 구성과 노드 분배 가이드](swarm.kr.md)를 먼저 확인하십시오. 단일 호스트 Compose를 replica만 늘려 배포하는 방식은 지원하지 않습니다.
